...
//volatile float vel_gain=0.00, pos_gain=0.0;
...
volatile int16_t compensation[M1_PULSE_NBR/8];
//CAN RX and TX buffers for the data interface (state and commands)
extern uint8_t rxCommandData[8];

extern union StatePacket{
	struct{
		int32_t q;
		int16_t q_dot;
		int16_t tau;
...
extern uint32_t latest_command_timestamp;

void unpack_command_buffer()
{
	// Decode 8-bit position gain
	pos_gain = rxCommandData[0];
	// Decode 8-bit velocity gain
	vel_gain = rxCommandData[1];
	// Decode 16 bit feed forward torque
	tau_ff  = (rxCommandData[6]<<8)|rxCommandData[7];
	// Decode 10-bit desired speed
	q_dot_d = (int32_t) ( (rxCommandData[2]) <<2 | rxCommandData[3] >>6 )-511;
	// Decode the 22-bit desired position
	q_d       = (int32_t)((rxCommandData[3]&0b00111111) <<16 |
				rxCommandData[4]<< 8 |
				rxCommandData[5])- 2097151;
}
...
//  MC_StopMotor1(void)
  ...
  while (1)
  {
	  a1 = MC_GetOccurredFaultsMotor1();
	  a3 = MC_GetCurrentFaultsMotor1();
//	  a2 = MC_GetPhaseCurrentAmplitudeMotor1();
//	  a3 = MC_GetCurrentFaultsMotor1();
    /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : CAN_TERM_Pin CAN_SHTD_Pin */
  ...
  /*Configure GPIO pin : USER_DEBUG_Pin */
